...
def 아이즈원_찾기(query):
    ...
    for tag in soup.select('.sh_blog_top'):
        ...
        for tag in soup.select('.detail_profile dd a'):
            #     print("{} : {}".format(tag.text, url+tag['href'] if tag["href"].find("https") else tag['href']))
            post_list.append({
                'name': tag.text,
                'url': url+tag['href'] if tag["href"].find("https") else tag['href'],
            })
